...
class Pembukuan_lelang extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // setting halaman
        $config['base_url'] = base_url('lelang/pembukuan-lelang/index/');
        $config['total_rows'] = $this->nota->countApp(sesi()['kdsatker'], sesi()['tahun'], 'L', 2);
        $config['per_page'] = 10;
        $config["num_links"] = 3;
        ...
        if ($data['urut']) {
            $data['nota'] = $this->nota->findApp(sesi()['kdsatker'], sesi()['tahun'], 'L', 2, $data['urut'], $limit, $offset);
        } else {
            $data['nota'] = $this->nota->getApp(sesi()['kdsatker'], sesi()['tahun'], 'L', 2, $limit, $offset);
        }
    ...
    public function process($id = null)
    {
        ...
        redirect('lelang/pembukuan-lelang');
    }
    ...
    public function detail($nota_id = null)
    {
        ...
        // setting halaman
        $config['base_url'] = base_url('lelang/pembukuan-lelang/detail/' . $nota_id . '/');
        ...
        $this->load->view('template/header', $data);
        $this->load->view('template/sidebar');
        $this->load->view('lelang/pembukuan_lelang/detail', $data);
        $this->load->view('template/footer');
    }
}
