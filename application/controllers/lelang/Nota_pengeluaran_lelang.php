...
class Nota_pengeluaran_lelang extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // setting halaman
        $config['base_url'] = base_url('lelang/nota-pengeluaran-lelang/index/');
        $config['total_rows'] = $this->nota->countDetail(sesi()['kdsatker'], sesi()['tahun'], 'K', 'L', 0);
        $config['per_page'] = 10;
        $config["num_links"] = 3;
        ...
        if ($data['urut']) {
            $data['nota'] = $this->nota->findDetail(sesi()['kdsatker'], sesi()['tahun'], 'K', 'L', 0, $data['urut'], $limit, $offset);
        } else {
            $data['nota'] = $this->nota->getDetail(sesi()['kdsatker'], sesi()['tahun'], 'K', 'L', 0, $limit, $offset);
        }
    ...
    public function create()
    {
        ...
        $this->load->view('template/header', $data);
        $this->load->view('template/sidebar');
        $this->load->view('lelang/nota_pengeluaran_lelang/create');
        $this->load->view('template/footer');
    }
    ...
    public function update($id = null, $jumlah = 1)
    {
        ...
        if ($jumlah > 0) {
            ...
            $this->load->view('template/header', $data);
            $this->load->view('template/sidebar');
            $this->load->view('lelang/nota_pengeluaran_lelang/update', $data);
            $this->load->view('template/footer');
        }
    }
    ...
    public function detail($nota_id = null)
    {
        ...
        // setting halaman
        $config['base_url'] = base_url('lelang/nota-pengeluaran-lelang/detail/' . $nota_id . '/');
        ...
        $this->load->view('template/header', $data);
        $this->load->view('template/sidebar');
        $this->load->view('lelang/nota_pengeluaran_lelang/detail', $data);
        $this->load->view('template/footer');
    }
    ...
    public function delete_detail($nota_id = null, $id = null)
    {
        ...
        redirect('lelang/nota-pengeluaran-lelang/detail/' . $nota_id . '');
    }
    ...
    public function create_detail($nota_id = null)
    {
        ...
        // setting halaman
        $config['base_url'] = base_url('lelang/nota-pengeluaran-lelang/create-detail/' . $nota_id . '/');
        $config['total_rows'] = $this->transaksi->countNota(sesi()['kdsatker'], sesi()['tahun'], $kdk, $kdj, 'K', 'L', 0);
        $config['per_page'] = 10;
        $config["num_links"] = 3;
        $this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
        ...
        // pilih tampilan data, semua atau berdasarkan pencarian
        if ($data['uraian']) {
            $data['transaksi'] = $this->transaksi->findNota(sesi()['kdsatker'], sesi()['tahun'], $kdk, $kdj, 'K', 'L', 0, $data['uraian'], $limit, $offset);
        } else {
            $data['transaksi'] = $this->transaksi->getNota(sesi()['kdsatker'], sesi()['tahun'], $kdk, $kdj, 'K', 'L', 0, $limit, $offset);
        }

        $this->load->view('template/header', $data);
        $this->load->view('template/sidebar');
        $this->load->view('lelang/nota_pengeluaran_lelang/create_detail', $data);
        $this->load->view('template/footer');
    }
    ...
    public function add_detail($nota_id = null, $id = null)
    {
        ...
        redirect('lelang/nota-pengeluaran-lelang/create-detail/' . $nota_id . '');
    }
}
